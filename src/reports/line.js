...
module.exports = function (config, result) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!config.token) return reject(new Error('config.token is required for the "line" report'))

      const url = new URL('https://notify-api.line.me/api/notify')

      if (config.onlyFailed === true && result.success === true) return resolve('[LINE] No Notification is required because eveything is fine :)')
