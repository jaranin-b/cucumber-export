...
describe('#report - SLACK', () => {
  test('Rejected if the config doesn\'t contain the url', () => {
    ...
    const config = {}
    const result = {}
    expect(Slack(config, result)).rejects.toThrow(new Error('config.url is required for the "slack" report'))
  })
  ...
  test('Resolved if the config only specify notification for failure', () => {
    ...
    expect(Slack(config, result)).resolves.toBe('[SLACK] No Notification is required because eveything is fine :)')
  ...
  test('Rejected if the request fail', () => {
    ...
    expect(Slack(config, testResult)).rejects.toThrow(new Errors.HTTP('SLACK REPORT', gotError))
